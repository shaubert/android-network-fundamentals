...
public class Main extends FragmentActivity {
    ...
    private void setupJournal() {
        RequestRepository repository = new RequestRepository(this, new DefaultRequestRecreator(this), RequestContract.Request.URI);
        DefaultExecutorBridge executorBridge = new DefaultExecutorBridge(this, DemoService.class);
        LoaderBasedRequestStateWatcher stateWatcher = new LoaderBasedRequestStateWatcher(repository, this, getSupportLoaderManager());
    ...
    private RequestStatusListener createStatusListener(final String filename, final ProgressDialog progressDialog) {
        return new RequestStatusListener() {
            ...
            public void onError(Request request) {
                progressDialog.dismiss();
                Toast.makeText(Main.this, ((RequestStateBase) request.getState()).getString("error"), Toast.LENGTH_LONG).show();
                journal.unregisterForUpdates(request);
            }
        };
    }
